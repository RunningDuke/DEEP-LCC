...
for i_data in range(data_total_number):
    ...
    if hdv_type == 1:
        # Driver Model: OVM
        file_path = 'hdv_ovm_' + data_str + '.mat'
        ...
        # Driver Model: IDM
        hdv_parameter_tmp = loadmat('hdv_idm.mat', mat_dtype=True)
        hdv_parameter = {"type": hdv_parameter_tmp["hdv_parameter"]["type"][0][0][0][0],
                         "v_max": hdv_parameter_tmp["hdv_parameter"]["v_max"][0][0][0][0],
    ...
    S[0, :, 1] = v_star * np.ones((1, n_vehicle + 1))
    ...
    for k in range(T - 1):
        ...
        acel = HDV_dynamics(S[k, :, :], hdv_parameter) - acel_noise + 2 * acel_noise * np.ones((n_vehicle, 1))

        S[k, 0, 2] = 0                                # the head vehicle
        S[k, 1:, 2] = acel.reshape((8,))              # all the vehicles using HDV model
        S[k, pos_cav, 2] = ud[:, k].reshape((2, 1))   # the CAVs

        S[k + 1, :, 1] = S[k, :, 1] + Tstep * S[k, :, 2]
        S[k + 1, 0, 1] = ed[0][k] + v_star            # the velocity of the head vehicle
        S[k + 1, :, 0] = S[k, :, 0] + Tstep * S[k, :, 1]

        yd[:, k] = measure_mixed_traffic(S[k, 1:, 1], S[k, :, 0], ID, v_star, s_star, measure_type).reshape((10,))
    ...
    mat_path = "./trajectory_data_collection/data" + data_str + "_" + str(i_data) + "_noiseLevel_" + str(acel_noise) + ".mat"
